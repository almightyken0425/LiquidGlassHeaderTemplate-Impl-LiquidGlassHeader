...
- **頂層 Product git：** `../../`
- **對側 Module Design git：** 尚無，模板庫視覺由本 repo 直接承載
- **對側 Module Spec git：** `no4_product_specs/no1_liquid_glass_header/`（目前為空殼）
- **本 Module Impl git：** 即本 repo（位於 `no6_product_development/no1_liquid_glass_header/`）
